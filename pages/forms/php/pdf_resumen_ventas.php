...
$pdf->Cell(230, 4, utf8_decode('SEDEM - SERVICIO DE DESARROLLO DE LA EMPRESAS PUBLICAS PRODUCTIVAS'), 1, 0, 'L');
$pdf->Ln(4);
$pdf->Cell(50, 4, 'Programa : ', 1, 0, 'R');
$pdf->Cell(230, 4, utf8_decode('OFICINA CENTRAL'), 1, 0, 'L');
$pdf->Ln(4);
$pdf->Cell(50, 4, 'Direccion : ', 1, 0, 'R');
$pdf->Cell(230, 4, utf8_decode('Av. Jaimes Freyre Nro. 9 Zona Sopocachi - La Paz'), 1, 0, 'L');
$a = 20;
$b = 90;
$c = 20;
$d = 20;
...
FROM  public.articulo a where a.id_empresa=" . $p);
    ...
    if (pg_num_rows($consulta) > 0) {
        $nn = 1;
        if ($p == 1) {
            $nombre_empresa = "LACTOSBOL - Empresa Boliviana de Almendras";
        }
        if ($p == 2) {
            $nombre_empresa = "EBA - Empresa Boliviana de Almendras Y Derivados";
        }
        if ($p == 3) {
            $nombre_empresa = "PAPELBOL - Empresa Publica Productiva Papeles de Bolivia";
        }
        $pdf->Ln(5);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(280, 5, utf8_decode(trim($nombre_empresa)), 1, 0, 'L');
        $num = 1;
        ...
        if ($consulta) {
            ...
            while ($array = pg_fetch_assoc($consulta)) {
                ...
                $cantidad = cantidad($array['id_articulo'], $_SESSION["fechainicial"], $_SESSION["fechafinal"]);


                if ($cantidad != 0) {
                    $precio = precio($array['id_articulo'], $_SESSION["fechainicial"], $_SESSION["fechafinal"]);
